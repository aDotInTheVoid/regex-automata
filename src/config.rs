...
///
/// In some cases, these options also apply to automaton construction. For
/// example, if the "allow invalid UTF-8" mode is enabled, then the implicit
/// `(?s:.)*?` prefix applied to unanchored regexes is permitted to match any
/// arbitrary byte. When "allow invalid UTF-8" mode is disabled (which is the
/// default), then the prefix is only allowed to match valid UTF-8.
