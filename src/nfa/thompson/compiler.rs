...
impl Config {
    ...
    fn get_allow_invalid_utf8(&self) -> bool {
        self.allow_invalid_utf8.unwrap_or(false)
    }
    ...
    pub(crate) fn overwrite(self, o: Config) -> Config {
        Config {
            allow_invalid_utf8: o
                .allow_invalid_utf8
                .or(self.allow_invalid_utf8),
...
impl Builder {
    ...
    pub fn configure(&mut self, config: Config) -> &mut Builder {
        // allow_invalid_utf8 is controlled by the syntax configuration, so
        // we don't let it get overridden here. We could permit setting it from
        // two places, but that seems more confusing.
        let allow_invalid_utf8 = self.config.allow_invalid_utf8;
        self.config = self.config.overwrite(config);
        self.config.allow_invalid_utf8 = allow_invalid_utf8;
        self
    }
    ...
    pub fn syntax(&mut self, config: crate::SyntaxConfig) -> &mut Builder {
        ...
        self.config.allow_invalid_utf8 = Some(config.get_allow_invalid_utf8());
        self
    }
}
...
impl Compiler {
    ...
    fn compile(&self, nfa: &mut NFA, expr: &Hir) -> Result<(), Error> {
        ...
        // all matches. This is an important DFA optimization. Without the
        // unanchored prefix, the DFA will correctly stop after the first
        // position. But if we added the unanchored prefix, an unmimized DFA
        // would continue searching. (Minimizing the DFA removes unreachable
        // states which in turn removes the unanchored prefix aspect of the DFA
        // that can never lead to a match.)
        //
        // When an unanchored prefix is not added, then the NFA's anchored
        // and unanchored start states are equivalent.
        let anchored =
            !self.config.get_unanchored_prefix() || expr.is_anchored_start();
        let unanchored_prefix = if anchored {
            self.c_empty()?
        } else {
            if self.config.get_allow_invalid_utf8() {
...
mod tests {
    ...
    use super::{Builder, Config, State, StateID, Transition, NFA};
