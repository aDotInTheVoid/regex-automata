...
impl<'a, S: StateID> Minimizer<'a, S> {
    pub fn new(dfa: &'a mut dense::DFA<Vec<S>, S>) -> Minimizer<'a, S> {
    ...
    pub fn run(mut self) {
        let alpha_len = self.dfa.alphabet_len();
        let as_state_id =
            |index: usize| -> S { S::from_usize(index * alpha_len) };
        let as_index = |id: S| -> usize { id.as_usize() / alpha_len };
        ...
        for (id, _) in self.dfa.states() {
            if state_to_part[as_index(id)] == id {
                minimal_ids[as_index(id)] = as_state_id(new_index);
                new_index += 1;
            }
        }
    ...
    fn initial_partitions(dfa: &dense::DFA<Vec<S>, S>) -> Vec<StateSet<S>> {
        ...
        // It's important for the quit state to come last. Namely, we want to
        // preserve it. In cases where there are no incoming transitions to
        // the quit state, then if it's first, it will never lead to forward
        // progress in Hopcroft's algorithm.
        if !is_quit.is_empty() {
            sets.push(is_quit);
        }
        sets
    ...
    fn incoming_transitions(dfa: &dense::DFA<Vec<S>, S>) -> Vec<Vec<Vec<S>>> {
        ...
        for (id, state) in dfa.states() {
            for (b, next) in state.transitions() {
                incoming[dfa.to_index(next)][b.as_usize()].push(id);
            }
        }
        incoming
    }
