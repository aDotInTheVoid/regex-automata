...
assert_eq!(matches, vec![Match::new(0, 10), Match::new(11, 21)]);
```
...
```
use regex_automata::{Match, dfa::{dense, Regex}};
...
let fwd: dense::DFA<&[u16], u16> = unsafe {
    dense::DFA::from_bytes_unchecked(&fwd_bytes)
};
let rev: dense::DFA<&[u16], u16> = unsafe {
    dense::DFA::from_bytes_unchecked(&rev_bytes)
};
...
* We specifically convert the dense DFA to a representation that uses `u16`
  ...
  [`dense::DFA::to_u16`](dense/struct.DFA.html#method.to_u16). While this isn't
...
```
use regex_automata::{Match, dfa::{sparse, Regex}};
...
let fwd: sparse::DFA<&[u8], u16> = unsafe {
    sparse::DFA::from_bytes_unchecked(&fwd_bytes)
};
let rev: sparse::DFA<&[u8], u16> = unsafe {
    sparse::DFA::from_bytes_unchecked(&rev_bytes)
};
// finally, reconstruct our regex
let re2 = Regex::from_dfas(fwd, rev);
...
pub use crate::dfa::automaton::Automaton;
#[cfg(feature = "std")]
pub use crate::dfa::error::{Error, ErrorKind};
pub use crate::dfa::regex::Regex;
#[cfg(feature = "std")]
