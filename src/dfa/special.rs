...
macro_rules! err {
    ($msg:expr) => {
        return Err(Error::deserialize($msg));
    };
}
...
//
// items. But state IDs are premultiplied, which means they are indices. So
// in order to take a state ID and index an array of accelerated structures,
// we need to do:
//
//     i = (state_id / alphabet_length) - special.min_accel
//
...
impl<S: StateID> Special<S> {
    ...
    /// Returns the number of bytes needed to serialize this structure to
    /// bytes.
    ...
    /// this will panic. To determine how much room is needed, use
    /// `Special::num_bytes`.
    pub fn to_bytes<E: ByteOrder>(&self, mut buf: &mut [u8]) {
        assert!(buf.len() >= Self::num_bytes());

        let state_size = size_of::<S>();

        write_state_id_bytes::<E, _>(&mut buf, self.max);
        buf = &mut buf[state_size..];
        write_state_id_bytes::<E, _>(&mut buf, self.quit_id);
        buf = &mut buf[state_size..];
        write_state_id_bytes::<E, _>(&mut buf, self.min_match);
        buf = &mut buf[state_size..];
        write_state_id_bytes::<E, _>(&mut buf, self.max_match);
        buf = &mut buf[state_size..];
        write_state_id_bytes::<E, _>(&mut buf, self.min_accel);
        buf = &mut buf[state_size..];
        write_state_id_bytes::<E, _>(&mut buf, self.max_accel);
        buf = &mut buf[state_size..];
        write_state_id_bytes::<E, _>(&mut buf, self.min_start);
        buf = &mut buf[state_size..];
        write_state_id_bytes::<E, _>(&mut buf, self.max_start);
    }

    /// Deserialize the given bytes into special state ranges. If the buffer
    ...
    pub fn from_bytes(mut buf: &[u8]) -> Result<Special<S>, Error> {
        if buf.len() < Self::num_bytes() {
            err!("buffer too small to deserialize special state info");
        }

        let state_size = size_of::<S>();

        let max = S::read_bytes(buf);
        buf = &buf[state_size..];
        let quit_id = S::read_bytes(buf);
        buf = &buf[state_size..];
        let min_match = S::read_bytes(buf);
        buf = &buf[state_size..];
        let max_match = S::read_bytes(buf);
        buf = &buf[state_size..];
        let min_accel = S::read_bytes(buf);
        buf = &buf[state_size..];
        let max_accel = S::read_bytes(buf);
        buf = &buf[state_size..];
        let min_start = S::read_bytes(buf);
        buf = &buf[state_size..];
        let max_start = S::read_bytes(buf);
        buf = &buf[state_size..];
    ...
    pub fn validate(&self) -> Result<(), Error> {
        ...
        if self.min_accel > self.max_match {
            err!("min_accel should not be greater than max_match");
        }
