...
impl Determinizer {
    ...
    pub fn run<S: StateID>(
        &self,
        nfa: &thompson::NFA,
        dfa: &mut dense::DFA<Vec<S>, S>,
    ) -> Result<(), Error> {
        let dead = Rc::new(State::dead());
        let quit = Rc::new(State::dead());
        let mut cache = HashMap::default();
        cache.insert(dead.clone(), dfa.dead_id());
...
impl<'a, S: StateID> Runner<'a, S> {
    ...
    fn run(mut self) -> Result<(), Error> {
        ...
        // At this point, we shuffle the "special" states in the final DFA to
        ...
        let mut is_match = BTreeSet::new();
        for (i, state) in self.builder_states.iter().enumerate() {
            if state.is_match() {
                is_match.insert(self.dfa.from_index(i));
            }
        }
        self.dfa.shuffle(&is_match);
        Ok(())
    }
    ...
    fn add_starts(
        ...
        let id = self.add_start(sparse, nfa_start, Start::AfterNonWordByte)?;
        self.dfa.set_start_state(Start::AfterNonWordByte, id);
        dfa_state_ids.push(id);

        if !self.nfa.has_word_boundary() {
            self.dfa.set_start_state(Start::AfterWordByte, id);
        } else {
            let id =
                self.add_start(sparse, nfa_start, Start::AfterWordByte)?;
            self.dfa.set_start_state(Start::AfterWordByte, id);
            dfa_state_ids.push(id);
        }
